...
// Animated hardware diagram — disk → DMA → memory + interrupt → CPU
      ...
      <svg viewBox={`0 0 ${W} ${H}`} width="100%" style={{ minWidth: '600px', maxWidth: '100%' }}>
        <defs>
          ...
          <radialGradient id="packet-grad" cx="50%" cy="50%" r="50%">
            <stop offset="0%" stopColor={color.pulse} stopOpacity="1" />
            <stop offset="60%" stopColor={color.pulse} stopOpacity="0.7" />
            <stop offset="100%" stopColor={color.pulse} stopOpacity="0" />
          </radialGradient>
          {/* IRQ pulse */}
          <linearGradient id="irq-grad" x1="0" y1="1" x2="0" y2="0">
          ...
          <marker id="irq-arrow" viewBox="0 0 10 10" refX="9" refY="5" markerWidth="6" markerHeight="6" orient="auto">
            <path d="M 0 0 L 10 5 L 0 10 z" fill={color.region.hardware.fg} />
          </marker>
          <marker id="dma-arrow" viewBox="0 0 10 10" refX="9" refY="5" markerWidth="5" markerHeight="5" orient="auto">
            <path d="M 0 0 L 10 5 L 0 10 z" fill={color.accent.primary} />
          </marker>
        </defs>
        ...
        <g transform="translate(20, 130)">
          ...
          {/* Platter circle */}
          <circle cx="75" cy="78" r="28" fill="none" stroke={color.region.hardware.accent} strokeWidth="1" opacity="0.5" />
          <circle cx="75" cy="78" r="20" fill="none" stroke={color.region.hardware.accent} strokeWidth="1" opacity="0.4" />
          <circle cx="75" cy="78" r="12" fill="none" stroke={color.region.hardware.accent} strokeWidth="1" opacity="0.3" />
          <circle cx="75" cy="78" r="3" fill={color.region.hardware.fg} />
          {/* Read head pointer */}
          <line x1="75" y1="78" x2="100" y2="63" stroke={color.region.hardware.fg} strokeWidth="1.5" strokeLinecap="round">
            <animateTransform attributeName="transform" type="rotate" from="0 75 78" to="360 75 78" dur="6s" repeatCount="indefinite" />
          </line>
          <text x="75" y="118" textAnchor="middle" fontFamily={font.family.mono} fontSize="8.5" fill={color.region.hardware.fg} opacity="0.7">
            sector 0x4290 +8
          </text>
        </g>
        ...
        <g transform="translate(190, 155)">
          {/* Bus track */}
          <rect width="320" height="64" rx="6" fill={color.bg.inset} stroke={color.accent.primary} strokeWidth="1" strokeOpacity="0.4" />
          <text x="160" y="14" textAnchor="middle" fontFamily={font.family.mono} fontSize="8.5" fill={color.accent.primary} fontWeight="700" letterSpacing="2">
            DMA BUS  ·  PCIe / AHCI
          </text>

          {/* Bus rail line */}
          <line x1="10" y1="36" x2="310" y2="36" stroke={color.accent.primary} strokeWidth="1.25" strokeOpacity="0.3" markerEnd="url(#dma-arrow)" />

          {/* Animated packets flowing left-to-right */}
          {[0, 1, 2, 3, 4].map(i => (
            <g key={i}>
              <circle r="6" fill="url(#packet-grad)">
        ...
        <g>
          <path
            d="M 95 130 Q 95 90 540 80"
            fill="none"
            stroke="url(#irq-grad)"
            strokeWidth="2"
            strokeDasharray="6 6"
            markerEnd="url(#irq-arrow)"
            opacity="0.85"
          >
          ...
          <circle r="4" fill={color.region.hardware.fg} opacity="0.95">
            <animateMotion path="M 95 130 Q 95 90 540 80" dur="1.4s" repeatCount="indefinite" />
            <animate attributeName="opacity" values="0;1;0" dur="1.4s" repeatCount="indefinite" />
          </circle>
          <text x="280" y="78" textAnchor="middle" fontFamily={font.family.mono} fontSize="9.5" fill={color.region.hardware.fg} fontWeight="700" letterSpacing="0.1em">
            IRQ — "I/O complete!"
          </text>
        </g>
      </svg>
