...
// The first 32 entries of x86_64 sys_call_table (with ioctl at slot 16)
...
export const IoctlKernelEntryHero: React.FC = () => (
  <div style={{ display: 'flex', flexDirection: 'column', gap: space[5] }}>
    ...
    <div>
      ...
      >
        %rax = 16 → sys_call_table[16] = <span style={{ color: color.pulse }}>sys_ioctl</span>
      </div>
    </div>

    {/* fd → file* lookup + sys_ioctl flow */}
    <div style={{ display: 'flex', gap: space[5], flexWrap: 'wrap', alignItems: 'flex-start' }}>
      ...
      <StructCard
        ...
        region="vfs"
        fields={[
          { label: 'fd[0]', value: 'tty (stdin)', highlight: true },
          { label: 'fd[1]', value: 'tty (stdout)' },
          { label: 'fd[2]', value: 'tty (stderr)' },
          { label: 'fd[3]', value: '*file (log)' },
        ]}
      />
      <div style={{ flex: 1, minWidth: '280px' }}>
        <SectionLabel accent={color.region.vfs.fg}>sys_ioctl()  ·  fs/ioctl.c:590</SectionLabel>
        <CodeBlock compact>{`SYSCALL_DEFINE3(ioctl, unsigned int, fd,
...
const IoctlKernelEntry: React.FC<DetailViewProps> = ({ node, region }) => (
  <DetailLayout node={node} region={region} heroLabel="syscall table dispatch: NR_ioctl = 16 → sys_ioctl()" hero={<IoctlKernelEntryHero />} />
);

export default IoctlKernelEntry;
